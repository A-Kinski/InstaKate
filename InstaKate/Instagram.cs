...
namespace InstaKate
{
    class Instagram : IDisposable
    {
        ...
        public static async Task<OAuthResponse> AuthByCredentialsAsync(string username, string password, InstagramConfig config, List<OAuth.Scope> scopes)
        {
            ...
            {
                if (await instagram.LoginAsync(username, password).ConfigureAwait(false))
                {
                    return await instagram.GetOauthResponse(config, scopes).ConfigureAwait(false);
                }
            }
        ...
        {
            var token = await GetAccessToken(config.ClientId, config.RedirectUri, BuildScopeForUri(scopes)).ConfigureAwait(false);
            ...
            var self = await users.GetSelf().ConfigureAwait(false);
            auth.User = self.Data;
        ...
        public Instagram()
        {
            ...
            m_Client.BaseAddress = new Uri("https://www.instagram.com/");
        ...
        public async Task<bool> LoginAsync(string username, string password)
        {
            ...
            await m_Client.GetAsync("/accounts/login/").ConfigureAwait(false);
            ...
            using (var response = await m_Client.SendAsync(request).ConfigureAwait(false))
            {
